...
> `.widget.js` 后缀名是一个约定而非规范内容，它的目的是让开发工具能够更好识别 Web Widget 应用。
...
> 应用生命周期来自于微前端框架 [single-spa](https://single-spa.js.org/) 的定义，这样可以确保 Web Widget 容器可以加载 [single-spa](https://single-spa.js.org/) 的应用。

Web Widget 元素会在不同的阶段主动触发这些应用生命周期：
...
```js
Web Widget.portalDestinations.define('dialog', () => {
