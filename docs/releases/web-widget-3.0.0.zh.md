...
- **Web Router 请求性能**：优化 Node 请求适配、路由匹配与中间件分发热路径；Node.js 22 基准中，默认路由吞吐量由 27,675 提升至 85,485 req/s（3.09 倍），Radix Tree 与 Manifest 模式分别提升至 2.02 倍和 2.99 倍（详见第 16 节）。
...
在同一基准套件与 Node.js v22.22.0 下，结果如下：

| 模式                     | 优化前（req/s） | 优化后（req/s） | 吞吐提升           |
| ------------------------ | --------------: | --------------: | ------------------ |
| Web Router（默认）       |          27,675 |          85,485 | 3.09 倍（+208.9%） |
| Web Router（Radix Tree） |          42,134 |          84,922 | 2.02 倍（+101.6%） |
| Web Router（Manifest）   |          16,890 |          50,525 | 2.99 倍（+199.1%） |

优化后的同轮对比中，默认与 Radix Tree 模式分别达到 85,485 和 84,922 req/s，位列测试框架前两名。基准吞吐量会受硬件和运行环境影响，应以同环境下的前后变化为准。
