...
- **HTML 模板 Widget 孤岛**：`@web-widget/html` 可通过 `scope` 匹配 `.ts` / `.js` 模块，自动注入 `render` 与 `container`，并可嵌入其他框架的 widget 作为交互孤岛（详见第 11 节）。
...
- **跨框架 props 类型推导**：React、Vue、Vue2 与 HTML 的 `container()` 统一使用 schema 层的结构化类型提取，覆盖 Preact、Svelte 等组件签名；无法自动识别时可显式使用 `container<Props>()`。
...
`defaults` 可以设置所有 Widget 容器的默认 `loading` 和 `renderTarget`。`renderTarget` 未配置时为 `light`；选择 `shadow` 后，构建工具向未显式配置 target 的 Widget renderer 注入 Shadow DOM target，并将 Widget CSS 从 document head 移入各自的 ShadowRoot。显式 `container()` options 优先于这些默认值。
...
`@web-widget/html` 声明 `WidgetAdapter` 协议。构建工具对带路由或 widget 标记的模块自动注入 `render`，并在导入 widget 时自动注入 `container`，使 HTML 模板可以直接嵌入其他框架的交互孤岛。
