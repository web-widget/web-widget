...
- **Web Router 请求性能**：优化 Node 请求适配、路由匹配与中间件分发热路径；Node.js 22 基准中，优化后的默认与 Manifest 模式三轮中位数分别为 83,974 和 50,192 req/s。相对历史单轮基线约为 3.03 倍和 2.97 倍（详见第 17 节；因统计口径不同，倍率仅作方向性参考）。
...
- **路由匹配**：为静态路径建立精确索引，并按首段为动态路由分桶；常见参数路由使用预编译 matcher，复杂 URLPattern 仍回退到标准实现以保持兼容。
...
优化后的同轮对比中，默认模式达到 83,974 req/s。基准吞吐量会受硬件和运行环境影响；判断回归时应比较相同配置下的多轮原始样本与中位数，而不是依赖单次排名。
