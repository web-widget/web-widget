...
- Widget 容器 props 迁移到 `widget` 对象（适用于 React、Vue、Vue2）。迁移示例：
  ...
  | `experimental_renderTarget` | 配置 `webWidgetPlugin.defaults.renderTarget` | 构建级默认配置            |
...
`defaults` 可以设置所有 Widget 容器的默认 `loading` 和 `renderTarget`。`renderTarget` 未配置时为 `light`；选择 `shadow` 后，构建工具向未显式配置 target 的 Widget renderer 注入 Shadow DOM target，并将 Widget CSS 从 document head 移入各自的 ShadowRoot。显式 `widget()` options 优先于这些默认值。
...
显式写法适用于需要精确控制类型推导或覆盖 `loading`、`renderTarget` 默认值的场景。两种写法在运行时等价。
