...
- **Widget 孤岛错误恢复与容器 API 重设计**：Widget 容器内部集成 `ErrorBoundary` 与 `Suspense`，Widget 渲染失败时错误被限制在单个孤岛内，不再导致整页空白。容器配置（`fallback`、`loading`、`serverOnly`、`clientOnly`）统一收进 `widget` prop，与 Widget 自身 props 命名空间隔离。所有框架适配器采用统一的 `fallback` 设计（`{ pending?, error? }`），`error` 省略时回退到 `pending`（详见第 11 节）。
- **统一的 `widget()` 导入 API**：移除 `asReactWidget` / `asHtmlWidget` 类型适配函数，所有跨框架 widget 导入统一使用 `widget()`。构建工具自动转换静态导入（`import Foo from './Foo@widget.vue'`）和显式 `widget(() => import(...))` 调用，注入 `import` 与 `name` 选项。同框架导入（如 React→React）享有完整的 props 类型推导，跨框架导入（如 Vue→React）通过 Vue `$props` 提取也实现了自动类型推导（详见第 16 节）。
...
- 移除 `asReactWidget` / `asHtmlWidget` 类型适配函数（含 `toReact` deprecated 别名）：跨框架 widget 导入统一使用 `widget()` 函数，构建工具自动注入 `import` 与 `name` 选项，无需手动类型适配。`widget()` 从各适配器包的 `./adapter` 子路径导入。
...
- Widget 容器 API 重设计（`@web-widget/react`、`@web-widget/vue`、`@web-widget/vue2`、`@web-widget/html`）：容器配置不再以扁平 prop 传递，改为统一收进 `widget` prop 对象。`experimental_loading` 重命名为 `loading`；`renderStage` 替换为互斥的 `serverOnly` / `clientOnly` 布尔缩写；构建默认值通过 `webWidgetPlugin.defaults` 配置。
...
- `?as=jsx` / `?as=tsx` 查询参数与 `asReactWidget` / `asHtmlWidget` 类型适配函数已移除。跨框架 widget 导入统一使用 `widget()`，构建工具自动注入 `import` 与 `name` 选项。迁移示例：
...
3.0 早期版本中，跨框架 widget 导入需要手动调用类型适配函数（`asReactWidget`、`asHtmlWidget`），用户需显式书写泛型参数，且不同框架的适配函数签名不一致。本版本将这些函数统一替换为 `widget()`——一个既做运行时包装（集成 `Suspense` + `ErrorBoundary`）又承载类型推导的单一入口。
