...
- `toReact` 重命名为 `asReactWidget`：该函数仅做类型层面的适配（type cast），并非运行时转换，新命名更准确地反映其语义。`toReact` 已移除。
...
- 将 `?as=jsx` / `?as=tsx` 导入替换为 `asReactWidget()` 类型适配；`toReact` 已移除，使用 `asReactWidget` 替代。迁移示例：
