...
Web Widget 3.0.0 对齐 Vite 8，在不改动用户 `vite.config.ts` 的前提下升级了构建与开发体验：一次 `vite build` 同时产出 server 与 client、dev 下服务端改动自动刷新浏览器、CSS 合并以提升首屏渲染（FCP）、构建产物命名更可读，并新增 Shadow DOM SSR 与原生插槽支持。同时引入 `WidgetTransform` 协议将框架适配标准化，新框架接入无需编写专属构建插件。底层基于 Vite 8 Environment API 双环境模型、统一 Builder 编排与 ModuleRunner HMR 重写，并显著优化 Web Router 的生产请求热路径。同时改进了缓存中间件的错误处理与 HTTP/2 HEAD 请求的兼容性。
...
- **Widget 孤岛错误恢复与容器 API 重设计**：支持 fallback 的 Widget 容器内部集成 `ErrorBoundary` 与 `Suspense`，Widget 渲染失败时错误被限制在单个孤岛内，不再导致整页空白。容器配置统一收进 `widget` prop，与 Widget 自身 props 命名空间隔离。React、Vue 和 HTML 适配器采用统一的 `fallback` 设计（`{ pending?, error? }`），`error` 省略时回退到 `pending`；Vue 2 因没有 Suspense，不支持 `fallback`（详见第 11 节）。
- **统一的 `widget()` 导入 API**：弃用 `asReactWidget` / `asHtmlWidget` 类型适配函数（保留兼容导出），所有跨框架 widget 导入统一推荐使用 `widget()`。构建工具自动转换静态导入（`import Foo from './Foo@widget.vue'`）和显式 `widget(() => import(...))` 调用，注入 `import` 与 `name` 选项。同框架导入（如 React→React）享有完整的 props 类型推导，跨框架导入（如 Vue→React）通过 Vue `$props` 提取也实现了自动类型推导（详见第 16 节）。
...
- **Shadow DOM SSR 与原生插槽**：Widget 可在服务端输出 Shadow DOM 并在客户端恢复，CSS 隔离在各自的 ShadowRoot；框架适配器支持通过 `slot` 将可交互 Widget 直接投影到原生插槽（详见第 10 节）。
...
- **WidgetTransform 协议**：新增框架无关的构建转换协议。适配器包通过 `./transform` 导出文件匹配规则与运行时模块 ID，用户显式导入后交给构建工具，无需为每个框架编写专属构建插件（详见第 9 节）。
- **新增五种框架适配器**：新增 Preact、Solid、Svelte、Lit 与 Web Components 适配器；前三者支持 SSR 与客户端恢复，Lit 和 Web Components 以客户端渲染为主（详见第 9 节）。
- **HTML 模板 Widget 孤岛**：`@web-widget/html` 可通过 `scopes` 匹配 `.ts` / `.js` 模块，自动注入 `render()` 与 `widget()`，并可嵌入其他框架的 Widget 作为交互孤岛（详见第 12 节）。
- **HTML 模板 Suspense 流式渲染**：`@web-widget/html` 新增 `suspense()` 函数，支持渐进式渲染——异步内容 resolve 后在流中原位置替换 fallback，不阻塞后续内容（详见第 13 节）。
- **HTML 模板 lit-html 兼容 directives**：`@web-widget/html` 新增 `classMap`、`styleMap`、`ifDefined`、`when`、`join` 五个模板指令函数，API 签名与 lit-html 一致，降低从 lit-html 迁移成本（详见第 14 节）。
- **HTML 模板压缩 Vite 插件**：`@web-widget/html` 新增 `htmlCompress` Vite 插件，在构建时压缩 `html` 标签模板字面量中的冗余空白，无需引入第三方 HTML 压缩库（详见第 15 节）。
...
- **Web Router 请求性能**：优化 Node 请求适配、路由匹配与中间件分发热路径；Node.js 22 基准中，优化后的默认、Radix Tree 与 Manifest 模式三轮中位数分别为 83,974、84,051 和 50,192 req/s。相对历史单轮基线约为 3.03 倍、1.99 倍和 2.97 倍（详见第 17 节；因统计口径不同，倍率仅作方向性参考）。
...
- **流式 SSR shell 错误感知**：流式渲染（`progressive` 模式）下，所有框架（React、Vue、HTML）的 shell 级错误现在能被正确捕获并触发框架级 500 错误页，而非静默返回 200 导致客户端收到残缺页面。Shell 指所有不在 `Suspense` 边界内的内容——这些错误不可恢复，应返回 500；而 `Suspense` 内的错误仍通过 `fallback` 恢复（详见第 11 节）。
...
- **跨框架 props 类型推导**：React、Vue、Vue2 与 HTML 的 `widget()` 统一使用 schema 层的结构化类型提取，覆盖 Preact、Svelte 等组件签名；无法自动识别时可显式使用 `widget<Props>()`。
...
- 弃用 `asReactWidget` / `asHtmlWidget` 类型适配函数（含 `toReact` deprecated 别名），并保留兼容导出：跨框架 widget 导入统一推荐使用 `widget()` 函数，构建工具自动注入 `import` 与 `name` 选项，无需手动类型适配。`widget()` 从各适配器包的 `./adapter` 子路径导入。
- 移除各框架的专属 Vite 插件（`reactWebWidgetPlugin` / `vueWebWidgetPlugin` / `vue2WebWidgetPlugin`）及其 `./vite` 子路径导出：改用 `webWidgetPlugin({ transforms: [...] })` 统一配置，并从适配器包的 `./transform` 子路径显式导入转换定义。适配器包不再依赖 `@web-widget/vite-plugin` 和 `vite` 作为 peerDependency。
- Widget 模块必须提供默认导出，`webWidgetPlugin` 才会自动注入 `render()`；仅包含命名导出的 `@widget` 模块不再参与渲染转换，需将组件改为默认导出。
- 框架适配器包入口分离（`@web-widget/react`、`@web-widget/vue`、`@web-widget/vue2`、`@web-widget/html`）：`.` 入口不再重新导出 `@web-widget/helpers`。用户 API（`defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等）改为从 `@web-widget/helpers` 导入，运行时函数（`render()`、`widget()`、`createVueRender()`）从 `./adapter` 子路径导入。`@web-widget/html` 的 `render()` 改为从 `./adapter` 子路径导入。
- Widget 容器 API 重设计（`@web-widget/react`、`@web-widget/vue`、`@web-widget/vue2`、`@web-widget/html`）：容器配置不再以扁平 prop 传递，改为统一收进 `widget` prop 对象。`experimental_loading` 重命名为 `loading`；渲染模式统一使用互斥的 `serverOnly` / `clientOnly` 布尔值；构建默认值通过 `webWidgetPlugin.defaults` 配置。
...
- 框架适配插件迁移到新的 `webWidgetPlugin({ transforms: [...] })` API（详见第 9 节）。迁移示例：
...
- `?as=jsx` / `?as=tsx` 查询参数已移除；`asReactWidget` / `asHtmlWidget` 类型适配函数已弃用并保留兼容导出。跨框架 widget 导入统一推荐使用 `widget()`，构建工具自动注入 `import` 与 `name` 选项。迁移示例：
...
- 此前从 `@web-widget/react` 或 `@web-widget/html` 导入的用户 API（`defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等）改为从 `@web-widget/helpers` 导入。运行时函数（`render()`、`widget()`）改为从 `@web-widget/react/adapter` 导入。`@web-widget/html` 的 `render()` 改为从 `@web-widget/html/adapter` 导入。迁移示例：

  ```diff
  - import { defineRouteComponent, defineMeta } from '@web-widget/react';
  + import { defineRouteComponent, defineMeta } from '@web-widget/helpers';

  - import { render, widget } from '@web-widget/react';
  + import { render, widget } from '@web-widget/react/adapter';
  ```

- **`@web-widget/html` 路由迁移**：`@route.ts` / `.route.ts` 文件可直接自动注入 `render()`，无需改为 `.html.ts`；若其他适配器也匹配 `.ts`，使用 `scopes` 消歧。同时移除手动 `export { render }`。`@layout.ts` 不受 adapter 影响，仍需手动从 `@web-widget/html/adapter` 导入并导出 `render`。迁移示例：
...
- Widget 容器 props 迁移到 `widget` 对象（适用于 React、Vue、Vue2）。迁移示例：
  ...
  | 仅服务端渲染                | `widget={{ serverOnly: true }}`      | 与 `clientOnly` 互斥      |
  | 仅客户端渲染                | `widget={{ clientOnly: true }}`      | 与 `serverOnly` 互斥      |
  | `experimental_renderTarget` | 配置 `webWidgetPlugin.defaults.root` | 构建级默认配置            |
...
| Web Router 请求性能   | 请求适配、路由匹配与分发走通用路径，产生较多临时对象               | 延迟实体化 Request/Response + 静态路由索引 + 单处理器同步分发                              |
...
| 框架适配              | 每个框架需编写专属 Vite 插件（`reactWebWidgetPlugin` 等）          | `WidgetTransform` 协议：显式导入 `./transform`，`webWidgetPlugin({ transforms })` 统一集成 |
...
## 9. WidgetTransform 协议

此前每个 UI 框架（React、Vue、Vue2）都需要提供专门的 Vite 插件（`reactWebWidgetPlugin`、`vueWebWidgetPlugin` 等），逻辑相似却互不兼容。3.0 引入 `WidgetTransform` 协议将转换逻辑标准化：适配器包通过 `./transform` 子路径导出框架标识、文件扩展名与完整的运行时模块 ID，构建工具接收显式导入的转换定义后完成文件匹配与渲染注入。

除 React、Vue、Vue2 与 HTML 外，3.0 还新增 `@web-widget/preact`、`@web-widget/solid`、`@web-widget/svelte`、`@web-widget/lit` 和 `@web-widget/web-components`。Preact、Solid、Svelte 支持 SSR 与客户端恢复；Lit 和 Web Components 为 CSR 适配器，服务端仅输出容器。
...
用户在 `vite.config.ts` 中显式导入转换定义即可，无需为每个框架单独配置插件：
...
`defaults` 可以设置所有 Widget 容器的默认 `loading` 和 `root`。`loading` 默认使用 `auto`：优先加载可见或发生交互的 Widget，并在浏览器空闲时加载其余 Widget；`root` 未配置时为 `light`。选择 `shadow` 后，构建工具向未显式配置根模式的 Widget renderer 注入该配置，并将 Widget CSS 从 document head 移入各自的 ShadowRoot。显式 `widget()` options 优先于这些默认值。

`scopes` 是目录路径列表，用于扩展名冲突时消歧义（如 Vue 2 与 Vue 3 共存）；文件位于任一指定目录下即可匹配。完整设计见 [RFC](../../rfcs/build-transformation-protocol.zh.md)。

## 10. Shadow DOM SSR 与原生插槽
...
容器配置（包括 `loading`、`serverOnly`、`clientOnly`，以及适配器支持时的 `fallback`）全部收进 `widget` 对象，彻底避免与 widget 自身 props 的命名冲突。

React、Vue 和 HTML 适配器采用统一的 `fallback` 设计：`fallback` 接受 UI 值或 `{ pending?, error? }` 对象，`error` 省略时回退到 `pending`。Vue 2 因没有 Suspense，不支持 `fallback`。
...
## 12. HTML 模板 Widget 孤岛

`@web-widget/html/transform` 导出 `WidgetTransform` 定义。构建工具对带路由或 Widget 标记的模块自动注入 `render()`，并在导入 Widget 时自动注入 `widget()`，使 HTML 模板可以直接嵌入其他框架的交互孤岛。

```typescript
// Page@route.ts — 自动注入 render()
...
容器 API 与 React 适配器对齐：Widget 自身 props 直接展开，容器配置（`loading`、`serverOnly`、`clientOnly`）收进 `widget` prop。`widget()` 从模块的默认导出自动推导 props 类型，HTML 模板中调用时享有完整的类型检查。
...
`widget()` 内部自动组合 `fallback(suspense(...), errorFn)`。流式协议使用 `$H` 前缀（`$HRC`、`HS:0`、`HB:0`）与 React 的 `$RC` 协议完全隔离，两者可在同一页面安全共存。

完整设计见 [RFC：HTML 模板 Suspense 渐进式渲染](../../rfcs/html-suspense.zh.md)。

## 14. HTML 模板 lit-html 兼容 directives
...
## 16. 统一的 `widget()` 导入 API

3.0 早期版本中，跨框架 widget 导入需要手动调用类型适配函数（`asReactWidget`、`asHtmlWidget`），用户需显式书写泛型参数，且不同框架的适配函数签名不一致。本版本以 `widget()` 统一替代这些函数的用途；旧函数仅作为已弃用的兼容导出保留。`widget()` 是一个既做运行时包装（集成 `Suspense` + `ErrorBoundary`）又承载类型推导的单一入口。
...
```typescript
// 构建工具自动转换为 widget() 调用，注入 import 与 name 选项
import VueCounter from './Counter@widget.vue';
```

**显式 `widget()` 调用**（跨框架类型安全场景）：
...
显式写法适用于需要精确控制类型推导或覆盖 `loading`、`root` 默认值的场景。两种写法在运行时等价。

### 构建时自动注入

`import-render` Vite 插件在 `transform` 阶段扫描源码，对两种写法自动注入 `import`（客户端 chunk 路径）与 `name`（组件名，用于 `<web-widget>` 元素的 `name` 属性）选项：

- **静态导入**：`import Foo from './Foo@widget.vue'` → `const Foo = widget(() => import(...), { import: "...", name: "Foo" })`
- **显式调用**：`widget(() => import('...'))` → `widget(() => import('...'), { import: "...", name: "Foo" })`，选项注入或合并到已有 options 对象
...
`widget()` 通过 `ExtractModuleProps<M>` 从模块的默认导出自动推导 props 类型：
...
## 17. Web Router 请求热路径性能

本次优化聚焦生产环境的每请求固定成本，不改变路由匹配语义与中间件 API：

- **Node 请求适配**：Node adapter 将轻量请求源直接交给 Web Router，仅在业务代码读取 `context.request` / `originalRequest` 时才实体化标准 `Request`；常见响应同样延迟创建完整 Web `Response`，减少 headers 与 body 的重复转换。
- **中间件分发**：单处理器请求走同步快速路径，多处理器链按需创建 `next()`；`state`、`rewrite`、`waitUntil` 与重写循环检测等低频状态均延迟分配。
- **路由匹配**：URLPattern 与 Radix Tree router 为静态路径建立精确索引，并按首段为动态路由分桶；常见参数路由使用预编译 matcher，复杂 URLPattern 仍回退到标准实现以保持兼容。
- **基础开销**：构建目标提升至 ES2022，避免将私有字段转译为每次访问 WeakMap；URL pathname 提取与参数解码在无转义字符时跳过正则和 `decodeURIComponent`。

在同一基准套件与 Node.js v22.22.0 下，优化后结果采用 3 轮中位数；每轮先以相同请求组合预热 2 秒，再测量 5 秒（50 connections、pipelining 4）。优化前数据来自同一环境的历史单轮报告，未采用相同的预热与多轮统计，因此下表倍率仅用于展示优化幅度，不应视为严格的同口径对照：

| 模式                     | 优化前（req/s） | 优化后（req/s） | 吞吐提升           |
| ------------------------ | --------------: | --------------: | ------------------ |
| Web Router（默认）       |          27,675 |          83,974 | 3.03 倍（+203.4%） |
| Web Router（Radix Tree） |          42,134 |          84,051 | 1.99 倍（+99.5%）  |
| Web Router（Manifest）   |          16,890 |          50,192 | 2.97 倍（+197.2%） |

优化后的同轮对比中，Radix Tree 与默认模式分别达到 84,051 和 83,974 req/s，位列测试框架前两名，差异不足 0.1%。基准吞吐量会受硬件和运行环境影响；判断回归时应比较相同配置下的多轮原始样本与中位数，而不是依赖单次排名。
