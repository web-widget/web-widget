...
- `@web-widget/react` 包入口分离：`.` 入口仅导出用户 API（`defineRouteComponent`、`defineMeta` 等），运行时代码（`render`、`defineWebWidget`、`useWidgetState`）移至 `./runtime` 子路径。此前从 `@web-widget/react` 导入运行时 API 的代码需改为从 `@web-widget/react/runtime` 导入。
...
- 若此前从 `@web-widget/react` 导入过 `render`、`defineWebWidget`、`useWidgetState` 等运行时 API，改为从 `@web-widget/react/runtime` 导入。迁移示例：
  ...
  `defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等用户 API 仍从 `@web-widget/react` 导入，无需改动。
