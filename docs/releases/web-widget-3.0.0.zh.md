...
`defaults` 可以设置所有 Widget 容器的默认 `loading` 和 `root`。`root` 未配置时为 `light`；选择 `shadow` 后，构建工具向未显式配置根模式的 Widget renderer 注入该配置，并将 Widget CSS 从 document head 移入各自的 ShadowRoot。显式 `widget()` options 优先于这些默认值。
