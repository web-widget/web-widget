...
- 框架适配器包入口分离（`@web-widget/react`、`@web-widget/vue`、`@web-widget/vue2`）：`.` 入口不再重新导出 `@web-widget/helpers`。用户 API（`defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等）改为从 `@web-widget/helpers` 导入，运行时代码（`render`、`container`、`createVueRender`、`useWidgetState`）从 `./runtime` 子路径导入。`asReactWidget`（及 deprecated 别名 `toReact`）仍从 `@web-widget/vue` 和 `@web-widget/vue2` 的 `.` 入口导出。
...
- 此前从 `@web-widget/react` 导入的用户 API（`defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等）改为从 `@web-widget/helpers` 导入。运行时 API（`render`、`container`、`useWidgetState`）改为从 `@web-widget/react/runtime` 导入。迁移示例：
