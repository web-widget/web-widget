...
- **HTML 模板 Widget 孤岛**：`@web-widget/html` 可通过 `scope` 匹配 `.ts` / `.js` 模块，自动注入 `render()` 与 `widget()`，并可嵌入其他框架的 Widget 作为交互孤岛（详见第 12 节）。
...
- **`@web-widget/html` 路由迁移**：`@route.ts` / `.route.ts` 文件可直接自动注入 `render()`，无需改为 `.html.ts`；若其他适配器也匹配 `.ts`，使用 `scope` 消歧。同时移除手动 `export { render }`。`@layout.ts` 不受 adapter 影响，仍需手动从 `@web-widget/html/adapter` 导入并导出 `render`。迁移示例：
...
`scope` 是目录路径列表，用于扩展名冲突时消歧义（如 Vue2 与 Vue3 共存）；文件位于任一指定目录下即可匹配。完整设计见 [RFC](../../rfcs/build-transformation-protocol.zh.md)。
