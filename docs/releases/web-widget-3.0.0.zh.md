...
Web Widget 3.0.0 对齐 Vite 8，在不改动用户 `vite.config.ts` 的前提下升级了构建与开发体验：一次 `vite build` 同时产出 server 与 client、dev 下服务端改动自动刷新浏览器、CSS 合并以提升首屏渲染（FCP）、构建产物命名更可读。同时引入 `WidgetAdapter` 协议将框架适配标准化，新框架接入无需编写专属构建插件。底层基于 Vite 8 Environment API 双环境模型、统一 Builder 编排与 ModuleRunner HMR 重写，并显著优化 Web Router 的生产请求热路径。同时改进了缓存中间件的错误处理与 HTTP/2 HEAD 请求的兼容性。
...
- **统一的 `widget()` 导入 API**：移除 `asReactWidget` / `asHtmlWidget` 类型适配函数，所有跨框架 widget 导入统一使用 `widget()`。构建工具自动转换静态导入（`import Foo from './Foo@widget.vue'`）和显式 `widget(() => import(...))` 调用，注入 `import` 与 `name` 选项。同框架导入（如 React→React）享有完整的 props 类型推导，跨框架导入（如 Vue→React）通过 Vue `$props` 提取也实现了自动类型推导（详见第 15 节）。
...
- **HTML 模板 Widget 孤岛**：`@web-widget/html` 可通过 `scope` 匹配 `.ts` / `.js` 模块，自动注入 `render()` 与 `widget()`，并可嵌入其他框架的 Widget 作为交互孤岛（详见第 11 节）。
...
- **Web Router 请求性能**：优化 Node 请求适配、路由匹配与中间件分发热路径；Node.js 22 基准中，优化后的默认、Radix Tree 与 Manifest 模式三轮中位数分别为 83,974、84,051 和 50,192 req/s。相对历史单轮基线约为 3.03 倍、1.99 倍和 2.97 倍（详见第 16 节；因统计口径不同，倍率仅作方向性参考）。
