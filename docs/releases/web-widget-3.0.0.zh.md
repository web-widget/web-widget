...
- 此前从 `@web-widget/react` 或 `@web-widget/html` 导入的用户 API（`defineRouteComponent`、`defineMeta`、`defineRouteHandler` 等）改为从 `@web-widget/helpers` 导入。运行时 API（`render`、`container`、`useWidgetState`）改为从 `@web-widget/react/adapter` 导入。`@web-widget/html` 的 `render` 改为从 `@web-widget/html/adapter` 导入（`html`、`widget`、`fallback` 等模板 API 仍从 `@web-widget/html` 导入）。迁移示例：
