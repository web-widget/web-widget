...
- **Dev 服务端改动自动刷新浏览器**：修改 server 代码后浏览器自动刷新以展示新的 SSR HTML，体验与 Remix / SvelteKit 一致（详见第 4 节）。
...
- 若代码中使用了 `Manifest.dev` 或 `StartOptions.dev`，改为 `exposeErrors`；如依赖生产环境的流式渲染，显式设置 `defaultRenderer.progressive: true`。
- 完整影响与可感知的变化详见第 6 节。
