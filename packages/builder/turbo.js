...
function replacePage(url, scrollPosition, callback) {
  ...
  prefetchNow(url)
    ...
    .then((html) => {
      ...
      scrollTo(
        0,
        scrollPosition || dom.querySelector(location.hash || "body").offsetTop
      );
      watchLinks();
    })
    .catch((e) => {
...
function replaceHtml(html) {
  ...
  queryAll(':not(link[rel="stylesheet"]):not(style)', prevHead).forEach((el) =>
    el.remove()
  );

  const prevHeadHrefs = new Set(
    queryAll("link", prevHead).map((el) => el.href)
  );
  toArray(head.children).forEach((el) => {
    if (
      el.tagName !== "LINK" ||
      el.rel !== "stylesheet" ||
      !prevHeadHrefs.has(el.href)
    ) {
      adoptNode(el);
