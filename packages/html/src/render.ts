 ...
 * boundaries, fallback content is sent immediately and async
 * content is streamed (and swapped via `$HRC` script) when ready. This
 * enables progressive rendering where slow async content does not block
 * the streaming of subsequent synchronous content.
 */
export function renderToStream(html: HTML): ReadableStream {
  // Local context per render — no global state, concurrency-safe.
  const ctx: RenderContext = { deferreds: [], nextId: 0, errorStack: [] };
  const textEncoder = new TextEncoder();

  async function* generate(): AsyncGenerator<Uint8Array> {
    // Phase 1: Stream main template (Suspense outputs fallback + markers)
    for await (const chunk of html[RENDER](ctx)) {
      yield textEncoder.encode(chunk);
    }

    // Phase 2: Drain deferreds (batch loop supports nested Suspense)
    ...
    while (ctx.deferreds.length > 0) {
      ...
      // Resolve all deferreds concurrently, output in completion order.
      // Each deferred's output is buffered atomically to prevent
      // interleaving with other concurrent deferreds.
    ...
    // No pop needed — ctx is a local variable, GC'd when generator ends.
  }
