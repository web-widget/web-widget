...
将双环境构建顺序从传统的 client → server 反转为 server → client（`runRouterBuildApp`，见 [server-output.ts](../src/router/server-output.ts)）：
...
server transform 不再内联 client 资产 URL，改为通过虚拟模块 `virtual:web-widget-server-assets`（见 [server-assets-plugin.ts](../src/router/server-assets-plugin.ts)）间接引用：server 构建期只产出占位 `.server-assets.js`，client 构建完成后由 `buildApp: post` 钩子回写真实 `assetUrls` 与 `linkMap`，server 运行时按需动态 `import()` 查表。
...
│   ├── config.ts               # 配置解析与默认值
│   ├── ensure-convention-files.ts # 约定文件 auto-ensure
│   ├── build-entry-points.ts   # routemap → client/server rolldown input + chunk 命名
│   ├── collect-route-assets.ts # route/fallback 引用图扫描（Direction A）
│   ├── server-assets-module.ts # virtual:web-widget-server-assets 虚拟模块
│   ├── css-merge.ts           # CSS 合并与内联（lightningcss url rebase）
│   ├── manifest-links.ts       # client manifest → <link> 描述符
│   └── routemap-manifest-code.ts # routemap → manifest 代码生成
├── router/
│   ├── host.ts                 # RouterPluginHost + WebRouterPlugin.api
│   ├── index.ts                # config / configEnvironment / builder
│   ├── server-entry.ts         # import.meta.framework transform + HMR accept
│   ├── server-output.ts        # buildApp、generateBundle、client manifest 捕获
│   ├── server-assets-plugin.ts # server-assets 虚拟模块 + 数据回写
│   ├── skip-server-css.ts      # server 构建跳过 CSS
│   ├── remove-async-hooks.ts   # 移除异步 hooks
│   ├── import-action.ts        # client @action 导入
│   └── preview.ts              # preview server 配置
├── widget/
│   ├── index.ts                # webWidgetPlugin
│   ├── export-render.ts        # server meta.link 注入
│   └── import-render.ts        # widget import 属性改写
├── dev/
│   ├── index.ts                # dev middleware
│   ├── meta.ts                 # dev CSS 收集
│   ├── warmup.ts               # server 预热
│   ├── resolve-dev-origin.ts   # dev/preview origin 解析
│   ├── module-source.ts        # x-module-source header
│   ├── dev-server-cache.ts     # revision 缓存
│   ├── server-invalidation.ts  # server 模块失效
│   ├── server-full-reload-plugin.ts # hotUpdate → full-reload
│   └── routing/                # 文件系统路由
...
- `internal/collect-route-assets.test.ts`：Direction A 引用图扫描（含 CSS `@import` 误匹配修复）
- `internal/manifest-links.test.ts`：client manifest → `<link>` 描述符
- `internal/css-merge.test.ts`：CSS 合并与内联（阈值判定、url rebase、策略覆盖）
- `router/host.test.ts`：RouterPluginHost 状态共享
- `router/skip-server-css.test.ts`：server 构建跳过 CSS
...
- `dev/server-invalidation.test.ts`：server 模块失效
...
对每个构建 entry（route module 与 widget module），按其引用的全部 CSS 文件总大小做二元决策——总大小 ≤ 阈值则内联为单个 `<style>`，否则合并为单个外部 CSS（`assets/css-[hash].css`）。每个 entry 最终只产生**一个** `<style>` **或** 一个 `<link>`，从结构上避免级联顺序问题。外部 CSS（`http://` / `https://`）与非 stylesheet link（如 `modulepreload`）不参与决策，原样保留。

```ts
webRouterPlugin({
  css: {
    inlineStrategy: 'auto', // 'auto'（按阈值）| 'always'（始终内联）| 'never'（始终外链）
    inlineThreshold: 8192, // 字节，默认 8 KB
  },
});
```

处理发生在 `buildServerAssetsData`（client 构建完成后），核心逻辑见 [css-merge.ts](../src/internal/css-merge.ts)。合并多个 CSS 时，各文件内部的相对 `url()` 路径会因基准目录变化而失效，用 `lightningcss`（Vite 传递依赖，已提升为直接依赖）在 transform 阶段重写为绝对路径。`ServerAssetsData` 新增 `styleMap` 字段，与 `linkMap` 对应；server 运行时通过 `resolveStyle(routeId)` 查表，将内联 CSS 注入 `meta.style`。
