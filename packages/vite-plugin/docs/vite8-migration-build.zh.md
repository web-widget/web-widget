...
| 代码组织              | 单文件 `entry.ts` 承载大部分逻辑                   | 拆分为 `src/plugins/` + 共享 `RouterPluginHost`            |
...
Preview：`src/preview.ts` 使用 `resolvePreviewOrigin()`，顺序为 `resolvedUrls` → `config.preview` 回退（preview 无 `origin` 选项）。
...
- SSR environment 不是 `RunnableDevEnvironment`（catch 后打日志，但 router middleware 不会挂载）
...
- 主要代码：`packages/vite-plugin/src/plugins/`、`environment.ts`、`router-plugin-host.ts`、`dev/index.ts`、`dev/meta.ts`、`dev/module-source.ts`、`dev/dev-server-cache.ts`、`dev/warmup.ts`、`dev/resolve-dev-origin.ts`、`dev/routing/index.ts`、`dev/ssr-full-reload-plugin.ts`
