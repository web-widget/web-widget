...
- 主要代码：`packages/vite-plugin/src/router/`、`internal/`（含 `ensure-convention-files.ts`、`build-entry-points.ts`、`config.ts`）、`dev/index.ts`、`dev/meta.ts`、`dev/module-source.ts`、`dev/dev-server-cache.ts`、`dev/warmup.ts`、`dev/resolve-dev-origin.ts`、`dev/routing/index.ts`、`dev/server-full-reload-plugin.ts`
