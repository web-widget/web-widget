...
同一份 widget 源码在 client 与 ssr 两个 Vite 环境中会被编译成不同语义：client 产出真实 chunk，server 只保留对 client 资产的间接引用。
...
builder.build(client)   →  产出 hashed chunks + .manifest.json
        ↓
builder.build(ssr)      →  读 manifest，解析占位符，注入 meta
```
...
| `vite build --ssr` | 仅 ssr（需已有 client manifest） |
...
Server 构建第一阶段（同一 `import-render` transform，ssr 环境）写入 `asset://` 占位符：
...
- 每个环境（`client`、`ssr` 等）拥有独立的 `moduleGraph`、`pluginContainer`
- 生产构建通过 `ViteBuilder` 编排；`builder.build(client)` 后再 `builder.build(ssr)` 以传递资产清单，是常见模式
...
| ---------------------------------------- | -------------------------------------------------------- |
| `builder.buildApp`                       | `runRouterBuildApp`：client → ssr                        |
| `configEnvironment`                      | 为 client/ssr 注入独立 `outDir`、`rolldownOptions.input` |
| `applyToEnvironment`                     | server entry、manifest 解析、`meta` 注入仅 ssr           |
| `RunnableDevEnvironment.runner.import()` | dev 每请求加载 server entry                              |
| `sharedDuringBuild`                      | `RouterPluginHost` 经 `plugin.api` 跨插件共享            |

当前唯一偏「传统」之处：ssr 通过 `fs.readFile` 读取磁盘上的 client manifest（`getManifest`），而非内存传递。这在 Remix、SvelteKit 等框架中仍很常见；Vite 社区正在讨论将 manifest 挂到 `BuildEnvironment` 上，但尚未成为稳定 API。
...
`@vitejs/plugin-rsc` 基于 Vite Environment API，定义 三个 生产环境（比 Web Widget 的 client + ssr 多一个 `rsc`）：

| 环境   | 条件 / 职责                                                                             |
| ------ | --------------------------------------------------------------------------------------- |
| rsc    | `react-server` 条件；`renderToReadableStream` 序列化 RSC payload；处理 Server Functions |
| ssr    | 反序列化 RSC stream → React VDOM → `react-dom/server` 输出 HTML                         |
...
| ----------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------- |
| 环境数量                | rsc + ssr + client（3）                                                 | client + ssr（2）                                                 |
...
2. plugin-rsc 显式拆出 `rsc` 环境处理 `react-server` 条件；Web Widget 在 ssr 环境内完成服务端渲染，无独立第三环境。
...
`@vitejs/plugin-react` 仅处理 React 编译与 HMR，不定义 client/server 组件边界，也不编排双环境构建。全栈 SSR 场景通常由：
...
| Remix / React Router v7 | Route Module     | loader/action + 同构组件      | Vite client manifest → SSR 注入 assets               |
| SvelteKit               | Svelte 组件      | `.server` / 自动 client 检测  | client manifest → `<script>` / `<link>`              |
| Nuxt 3                  | Vue 组件 + Nitro | 自动 server/client 拆分       | Vite client manifest → Nitro SSR                     |
...
| ----- | -------------------- | ------------------------------------------ |
| Astro | Islands (`client:*`) | SSR 静态外壳 + 按需加载 client chunk       |
...
| Server 持有什么 | Client Reference ID                      | `asset://` → hashed URL          |
...
1. 内存 manifest：client build 结束后写入 `RouterPluginHost`，ssr 优先读内存；待 Vite 提供 `environment.manifest` 后可进一步对齐。
2. Virtual module：例如 `virtual:web-widget-client-manifest`，仅在 ssr 环境 resolve，集中 manifest 读取逻辑。
...
| 其它元框架怎么做？                           | 多数 Vite 全栈框架为 client manifest → ssr；RSC 系用协议 + module map                                 |
