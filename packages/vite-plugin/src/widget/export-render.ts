...
import { toNativeIdFilter } from '@/internal/module-id';
...
export function exportRenderPlugin({
  ...
  // Native hook filter: moves include/exclude matching to the Rust level.
  const nativeFilter = toNativeIdFilter(include, exclude);
        ...
        async handler(code, id) {
          ...
          if (extractFromExportDefault) {
            ...
            extractFromExportDefault.forEach((item) => {
              const itemFilter = createFilter(item.include, item.exclude);
              if (
