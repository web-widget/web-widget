...
type WidgetContainerConfig = {
  /** 加载态和错误态的占位 UI；error 省略时回退到 pending */
  fallback?:
    | HTML
    | {
        pending?: HTML;
        error?: HTML | ((error: unknown) => HTML);
      };
  /** 客户端加载策略 */
...
构建工具将 `import Counter from './Counter@widget.tsx'` 转换为 `widget()` 调用，运行时得到 `HtmlWidgetComponent`。显式调用 `widget()` 时，其泛型从模块默认导出提取 props，并返回适合 HTML 模板使用的组件类型：
...
// count 从 React 组件的 props 自动推导
html`<div>${Counter({ count: 1 })}</div>`;
```

约定式静态导入 `import Counter from './Counter@widget.tsx'` 由构建工具自动转换为相同的 `widget()` 调用。旧的 `asHtmlWidget` 仅作为兼容 API 保留并已弃用。
...
import { render, widget } from '@web-widget/html/adapter';
