...
Widget 是独立的客户端模块，构建后产出独立的 hashed chunk。前端 UI 组件通过 `defineWebWidget` 导入 Widget 模块：
...
核心挑战在于：transform 阶段生成 `defineWebWidget` 调用时，hashed chunk URL 尚不可知。需要一种机制在构建时或运行时填补这个信息缺口。
...
```javascript
// server transform 产物
defineWebWidget(() => import('./Counter@widget.tsx'), {
...
// Rolldown 解析后
defineWebWidget(() => import("./Counter@widget.tsx-HASH.js"), {
