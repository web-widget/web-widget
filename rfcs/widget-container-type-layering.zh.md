...
| 层级     | 当前类型                   | 实际用途                                              |
| -------- | -------------------------- | ----------------------------------------------------- |
| Schema   | `WidgetAdapterOptions`     | `widget(loader, options)` 的定义期默认配置            |
...
- 保持 `widget()` 的 props 自动推导和显式 `widget<Props>()` 兜底；
...
`WidgetContainer` 前缀只用于直接服务于 `widget()` 抽象的类型，不扩散到所有 Widget 类型：

- `Options` 表示 `widget(loader, options)` 的定义期参数；
...
`widget(loader, options)` 的第二个参数描述容器组件的默认配置，改名为 `WidgetContainerOptions`：
...
返回组件的 `widget` prop 使用 `WidgetContainerProps`。`Options` 专指 `widget()` 的函数参数，`Props` 专指组件调用位置，避免两个生命周期共享同一个后缀：
...
该接口的返回值保持 `unknown`。具体 adapter 的声明负责提供框架组件类型，具体实现函数仍使用小写 `widget()`。
...
当前 `components.ts` 同时包含公开类型、fallback 逻辑、内部 renderer wrapper 和 `widget()`。建议按职责逐步拆分：

```text
src/
  widget.ts          # widget() 与框架组件类型
  widget-options.ts  # WidgetContainerOptions 与框架化的 WidgetContainerProps
